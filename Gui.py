...
class HsiApp(tk.Frame):
    def __init__(self,master=None):
        ...
        map_button1 = tk.Button(master, text="Fry-CreateRaster", command=lambda:[Raster_Creation(life_stage="fry", method="hsi"), open_smth(canvas)])
        map_button1.grid(column=1, row=5)

        map_button2 = tk.Button(master, text="Spawning-CreateRaster", command=lambda:[Raster_Creation(life_stage="spawning", method="hsi"), open_smth(canvas)])
        map_button2.grid(column=1, row=6)
        #
        map_button3 = tk.Button(master, text="Juvenile-CreateRaster", command=lambda:[Raster_Creation(life_stage="juvenile", method="hsi"), open_smth(canvas)])
        map_button3.grid(column=2, row=5)
        #
        map_button4 = tk.Button(master, text="Adult-CreateRaster", command=lambda: [Raster_Creation(life_stage="adult", method="hsi"), open_smth(canvas)])
        ...
        map_button5 = tk.Button(master, text="Fry-CreateRaster", command=lambda: [Raster_Creation_Fuzzy(life_stage="fry", method="fuzzy_logic"), open_smth(canvas)])
        map_button5.grid(column=1, row=8)

        map_button6 = tk.Button(master, text="Spawning-CreateRaster", command=lambda:[Raster_Creation_Fuzzy(life_stage="spawning",  method="fuzzy_logic"), open_smth(canvas)])
        map_button6.grid(column=1, row=9)
        #
        map_button7 = tk.Button(master, text="Juvenile-CreateRaster", command=lambda:[Raster_Creation_Fuzzy(life_stage="juvenile", method="fuzzy_logic"), open_smth(canvas)])
        map_button7.grid(column=2, row=8)
        #
        map_button8 = tk.Button(master, text="Adult-CreateRaster", command=lambda: [Raster_Creation_Fuzzy(life_stage="adult",  method="fuzzy_logic"), open_smth(canvas)])
        ...
        def open_smth(canvas):
            ...
            my_image = ImageTk.PhotoImage(Image.fromarray((np.array(img) * 800).astype(np.uint8)))
            canvas.create_image(0, 0, image=my_image, anchor=NW)
            pass

        # def open_fuzzy(canvas):
        #     file = os.path.abspath("") + "\\habitat\\hsi_fuzzy.tif"
        #     global my_image
        #     img = Image.open(file).resize(img_size, resample=Image.NEAREST)
        #     my_image = ImageTk.PhotoImage(Image.fromarray((np.array(img) * 800).astype(np.uint8)))
        #     canvas.create_image(0, 0, image=my_image, anchor=NW)
