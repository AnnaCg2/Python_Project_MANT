...
x_habitat = np.arange(0, 1, .05)

# Generate fuzzy membership functions
#add import capabilities from json

velocity_lo = fuzz.trimf(velocity, [0, 0, 5])
velocity_md = fuzz.trimf(velocity, [0, 5, 10])
velocity_hi = fuzz.trimf(velocity, [5, 10, 10])
...
velocity_level_lo = fuzz.interp_membership(velocity, velocity_lo, 6.5)
velocity_level_md = fuzz.interp_membership(velocity, velocity_md, 6.5)
velocity_level_hi = fuzz.interp_membership(velocity, velocity_hi, 6.5)

depth_level_lo = fuzz.interp_membership(depth, depth_lo, 9.8)
depth_level_md = fuzz.interp_membership(depth, depth_md, 9.8)
depth_level_hi = fuzz.interp_membership(depth, depth_hi, 9.8)
